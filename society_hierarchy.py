...
class Citizen:
    ...
    def add_subordinate(self, subordinate: Citizen) -> None:
        ...
        self._subordinates = merge(new_subordinate, self._subordinates)
    ...
    def remove_subordinate(self, cid: int) -> None:
        ...
        True
        """
        for martian in self._subordinates:
            if martian.cid == cid:
                self._subordinates.remove(martian)
                martian.set_superior(None)
    ...
    def become_subordinate_to(self, superior: Optional[Citizen]) -> None:
        ...
        []
        """
        if self._superior is not None:  # if citizen has a superior
            self._superior.remove_subordinate(self.cid)  # removes itself
        self.set_superior(superior)  # sets superior of citizen
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if self.cid == cid:  # if the root is citizen you are looking for
            return self
        else:
            for subordinate in self._subordinates:  # go through all subs
                base_value = subordinate.get_citizen(cid)  # subs of subs
                if base_value:
                    return base_value
            return None
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        direct_subs = self.get_direct_subordinates()
        if not direct_subs:
            return []
        all_subs = direct_subs
        for sub in direct_subs:
            indirect_subs = sub.get_all_subordinates()
            all_subs = merge(all_subs, indirect_subs)
        return all_subs
    ...
    def get_society_head(self) -> Citizen:
        ...
        3
        """
        #  want to get to the root of the society
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        if self._superior is None:  # if this is the root
            return self
        else:
            if self.get_citizen(cid) in self.get_all_subordinates():
                return self
            elif self == self.get_citizen(cid):
                return self
            else:
    ...
    def get_district_name(self) -> str:
        ...
        """
        # Note: This method must call itself recursively
        if isinstance(self, DistrictLeader):  # if it is district leader
            return self._district_name  # return district name
        else:
            if self._superior is not None:  # make sure it is not root
                return self._superior.get_district_name()  # get higher district
        return ''
    ...
    def rename_district(self, district_name: str) -> None:
        ...
        # Note: This method must call itself recursively
        if isinstance(self, DistrictLeader):  # if it is district leader
            self.rename_district(district_name)  # rename district
        else:
    ...
    def get_highest_rated_subordinate(self) -> Citizen:
        ...
        top_rated = subs[0]
        for sub in subs:
            if sub.rating > top_rated.rating:
                top_rated = sub
        return top_rated
...
class Society:
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if isinstance(self._head.get_citizen(cid), Citizen):
    ...
    def get_all_citizens(self) -> List[Citizen]:
        ...
        True
        """
        all_subordinates = self.get_head().get_all_subordinates()
        head = [self.get_head()]
        return merge(all_subordinates, head)
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        if superior_id is None:
            if self.get_head() is not None:
                self.get_head().become_subordinate_to(citizen)
    ...
    def get_citizens_with_job(self, job: str) -> List[Citizen]:
        ...
        for citizen in self.get_all_citizens():
            if citizen.job == job:
                employee_list.append(citizen)

        half1 = employee_list[:len(employee_list) // 2]
        half2 = employee_list[len(employee_list) // 2:]
    ...
    def change_citizen_type(self, cid: int,
        ...
        citizen = self.get_citizen(cid)  # find the citizen
        if isinstance(citizen, DistrictLeader):  # if District leader -> Citizen
            new_citizen = Citizen(citizen.cid, citizen.manufacturer,
                                  citizen.model_year, citizen.job,
                                  citizen.rating)
            new_citizen.become_subordinate_to(citizen.get_superior())
            citizen.get_superior().remove_subordinate(citizen.cid)
            for sub in citizen.get_direct_subordinates():
                sub.become_subordinate_to(new_citizen)
            citizen._district_name = None
            return new_citizen
        else:  # Citizen -> District Leader
            new_dl = DistrictLeader(citizen.cid, citizen.manufacturer,
                                    citizen.model_year, citizen.job,
                                    citizen.rating, district_name)
            new_dl.become_subordinate_to(citizen.get_superior())
            citizen.get_superior().remove_subordinate(citizen.cid)
            for sub in citizen.get_direct_subordinates():
                sub.become_subordinate_to(new_dl)
            return new_dl
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        superior = citizen.get_superior()  # get superior
        citizen.job, superior.job = superior.job, citizen.job  # swap jobs
        citizen_subs = citizen.get_direct_subordinates()
        superior_subs = superior.get_direct_subordinates()
        superior_subs.remove(citizen)
        for sub in citizen_subs:
            sub.become_subordinate_to(superior)
        for sub in superior_subs:
            sub.become_subordinate_to(citizen)
        citizen.become_subordinate_to(superior.get_superior())
        ...
        if isinstance(superior, DistrictLeader):
            ...
            superior = self.change_citizen_type(superior.cid, None)
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        while (citizen.get_superior() is not None and citizen.rating >
                citizen.get_superior().rating and
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        """
        citizen = self.get_citizen(cid)
        if citizen is self._head:
            if not citizen.get_direct_subordinates():  # if no direct subs
                self._head = None
            else:  # if citizen is the head
                top = citizen.get_highest_rated_subordinate()
                citizen.remove_subordinate(top.cid)  # avoid double count
                subs = citizen.get_direct_subordinates()  #
                for sub in subs:
                    sub.become_subordinate_to(top)
                self.set_head(top)
        else:  # this is the first case
            citizen_subs = citizen.get_direct_subordinates()
            for sub in citizen_subs:
                sub.become_subordinate_to(citizen.get_superior())
            citizen.get_superior().remove_subordinate(citizen.cid)
...
class DistrictLeader(Citizen):
    ...
    def get_district_citizens(self) -> List[Citizen]:
        ...
        own_self = [self]
        return merge(own_self, subordinates)
